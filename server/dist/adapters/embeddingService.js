...
exports.embedTextoCompleto = embedTextoCompleto;
let _openai = null;
let OpenAIClass = null;
function resolveOpenAIClass() {
    if (OpenAIClass)
        return OpenAIClass;
    let mod;
    try {
        ...
        throw new Error("Pacote 'openai' não encontrado. Instale-o como dependência para gerar embeddings reais.");
    }
    const ctor = mod?.default ?? mod?.OpenAI ?? mod;
    if (typeof ctor !== "function") {
        throw new Error("Pacote 'openai' encontrado, mas não exporta um construtor válido.");
    }
    OpenAIClass = ctor;
    return OpenAIClass;
}
...
function fakeEmbedding(text, dim = EXPECTED_DIM || 256) {
    const te = new TextEncoder();
    const bytes = te.encode(text || "");
    const v = new Array(dim).fill(0);
    for (let i = 0; i < bytes.length; i++)
        v[i % dim] += (bytes[i] - 127) / 127;
    return unitNorm(v);
}
/** Gera embedding para um texto completo (com validações simples) */
async function embedTextoCompleto(texto, _tag) {
    ...
    if (EXPECTED_DIM && emb.length !== EXPECTED_DIM) {
        ...
        if (emb.length > EXPECTED_DIM)
            emb.length = EXPECTED_DIM;
        else {
            const filled = emb.slice();
            while (filled.length < EXPECTED_DIM)
                filled.push(0);
            return filled;
        }
    }
