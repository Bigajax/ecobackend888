...
function evalRule(rule, ctx) {
    ...
    try {
        ...
        // 1) comparações → 0/1
        r = r.replace(/(\d+(?:\.\d+)?)\s*(<=|>=|<|>|==|!=)\s*(\d+(?:\.\d+)?)/g, (_, a, op, b) => {
        ...
        const evalExpr = (s) => {
            while (/\([^()]*\)/.test(s))
                s = s.replace(/\(([^()]+)\)/g, (_, inner) => String(evalExpr(inner)));
            s = s.replace(/!\s*([01])/g, (_, x) => (x === '1' ? '0' : '1'));
            const and = (t) => t.split('||').map(x => x.trim().split(/&&/).every(y => y.trim() === '1') ? '1' : '0').some(v => v === '1') ? 1 : 0;
            return and(s);
        };
        return !!evalExpr(r);
    }
...
//# sourceMappingURL=ruleEval.js.map