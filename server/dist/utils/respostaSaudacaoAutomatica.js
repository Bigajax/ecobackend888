...
function getSaudacaoHorario() {
    ...
    if (hora < 6)
        return { saudacao: "Boa noite", energia: "silenciosa" };
    if (hora < 12)
        return { saudacao: "Bom dia", energia: "desperta" };
    if (hora < 18)
        return { saudacao: "Boa tarde", energia: "atenta" };
    return { saudacao: "Boa noite", energia: "recolhida" };
}
function respostaSaudacaoAutomatica({ messages, userName }) {
