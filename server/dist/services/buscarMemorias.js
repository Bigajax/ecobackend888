...
const supabaseAdmin_1 = require("../lib/supabaseAdmin"); // ✅ renomeado
...
async function buscarMemoriasSemelhantes(userIdOrNull, entradaOrOpts) {
    try {
        ...
        if (typeof entradaOrOpts === "string") {
            ...
            const requestedK = typeof entradaOrOpts.k === "number" ? Math.max(1, entradaOrOpts.k) : 4;
        ...
        // Client a usar
        const sb = supabaseClient ?? supabaseAdmin_1.supabase; // ✅ preferir injetado
        ...
        // ---------------------------
        const queryEmbedding = await (0, prepareQueryEmbedding_1.prepareQueryEmbedding)({ texto, userEmbedding });
        if (!queryEmbedding)
            return [];
        ...
        const call = async (db) => {
            const { data, error } = await sb.rpc("buscar_memorias_semelhantes_v2", {
            ...
            } // <- se seu tipo gerado não estiver atualizado
            );
            if (error) {
        ...
        return rows
            ...
            intensidade: typeof d.intensidade === "number"
                ? d.intensidade
                : d.intensidade != null
