...
exports.extrairTempoMencionado = extrairTempoMencionado;
exports.detectarPraticasRegulacao = detectarPraticasRegulacao;
exports.melhorPratica = melhorPratica;
...
function extrairTempoMencionado(texto) {
    const t = norm(texto);
...
function pontuarBox(t) {
    ...
    if (includesAny(tx, KW.box))
        s += 0.5;
    if (includesAny(tx, KW.pressaoCurta))
        s += 0.35;
 ...
 */
function detectarPraticasRegulacao(input, limiar = 0.6) {
    const resultados = [];
...
function melhorPratica(input, limiar = 0.6) {
    const r = detectarPraticasRegulacao(input, limiar);
    return r.length ? r[0] : null;
}
//# sourceMappingURL=regulacaoTriggers.js.map