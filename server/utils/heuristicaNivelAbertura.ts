...
export function heuristicaNivelAbertura(texto?: string): 1 | 2 | 3 {
  ...
  const isGreeting =
    /^(oi|ola|oie|opa|e ai|eae|bom dia|boa tarde|boa noite)\b/.test(t) && t.length <= 40;
  ...
  // flags semânticas
  const introspectiva = /(quero entender|quero explorar|tenho pensado|estou pensando|reflet(i|indo)|me percebo|percebi|como lido|entender melhor|faz sentido|o que isso diz)/.test(
    t,
  );
  const curiosidade = /(por que|porque|pq|explica|entender)/.test(t);
  const vulneravel =
    /(posso falar|dif[ii]cil de dizer|me sinto|sinto que|tenho sentido|quero abrir|preciso desabafar|posso compartilhar)/.test(
      t,
    );
  ...
  if (introspectiva) score += 2;
  if (curiosidade) score += 1;
  if (vulneravel) score += 1;

  // muito curto e sem sinais → 1
  if (wc <= 6 && !introspectiva && !vulneravel && !curiosidade) return 1;
...
export function heuristicaNivelAberturaDebug(texto?: string) {
  const raw = (texto || "").trim();
  const t = raw.toLowerCase().normalize("NFD").replace(/\p{Diacritic}/gu, "");
