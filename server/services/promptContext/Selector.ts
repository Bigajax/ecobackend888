...
import { Memoria, Heuristica, NivelNum } from "../../utils/types";
...
export function selecionarModulosBase({ nivel, intensidade, matriz, flags }:{
  ...
  return { selecionados: posGating, debug: { raw: dedup, inherited, specific, posGating, cortadosPorRegraOuIntensidade: cortados } };
}
...
const inCooldown = (uid?: string, arq?: string, secs=900) => {
  if (!uid || !arq) return false; const rec = lastTriggeredByUser.get(uid) ?? {};
  const last = rec[arq] ?? 0; return (Date.now()/1000 - last) < secs;
};
const mark = (uid?: string, arqs: string[] = []) => {
  if (!uid) return; const rec = lastTriggeredByUser.get(uid) ?? {};
  const now = Math.floor(Date.now()/1000); for (const a of arqs) rec[a] = now; lastTriggeredByUser.set(uid, rec);
};
...
export function selecionarExtras({
  ...
  const push = (x: ExtraCand) => { const i = cands.findIndex(c => c.arquivo === x.arquivo); if (i>=0){ if(x.score>cands[i].score) cands[i]=x; } else cands.push(x); };
  ...
  if (heuristicaAtiva?.arquivo && intensidade <= 6) push({ arquivo: heuristicaAtiva.arquivo, cat: "cognitivo", score: 2.5 });
  ...
  if (okFilo) {
    ...
    for (const e of (estoicosTriggerMap ?? []))
      if (e.gatilhos?.some((g: string) => txt.includes(normalizar(e))))
  ...
  for (const m of (emocionaisTriggerMap ?? [])) {
    ...
    if (minOK && (tagMatch || emoMatch || gatMatch)) push({ arquivo: m.arquivo, cat: "emocional", score: 3 + (intensidade>=7?1:0) });
  }
