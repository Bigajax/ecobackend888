...
interface ParallelFetchLike {
  run(params: {
    ...
    userId?: string;
    supabase?: any;
  }): Promise<{ heuristicas: any[]; userEmbedding: number[]; memsSemelhantes: any[] }>;
}
...
export async function loadConversationContext(
  ...
  const derivadosCacheKey = !shouldSkipDerivados && userId
    ? `derivados:${userId}`
    : null;
  ...
  const aberturaHibrida = derivados
    ? (() => {
        try {
          ...
          logger?.warn?.(
            `[derivadosLoader] insightAbertura falhou: ${(e as Error)?.message}`
          );
          return null;
        }
      })()
