...
-- Create meditation_feedback table
CREATE TABLE IF NOT EXISTS meditation_feedback (
  -- Identificadores
...
DROP TRIGGER IF EXISTS update_meditation_feedback_updated_at ON meditation_feedback;
...
COMMENT ON TABLE meditation_feedback IS 'Stores user feedback for meditation sessions with detailed metrics and context';
COMMENT ON COLUMN meditation_feedback.vote IS 'User satisfaction: positive or negative';
COMMENT ON COLUMN meditation_feedback.reasons IS 'Reasons for negative feedback (array of: too_long, hard_to_focus, voice_music, other)';
COMMENT ON COLUMN meditation_feedback.completion_percentage IS 'Percentage of meditation completed (0-100)';
COMMENT ON COLUMN meditation_feedback.user_id IS 'Authenticated user ID (NULL for guests)';
COMMENT ON COLUMN meditation_feedback.guest_id IS 'Guest session ID (NULL for authenticated users)';
